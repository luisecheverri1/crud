...
from reconfacial1.models import Persona, ReconocimientoFacial

@csrf_exempt
def reconocer_rostros(request):

    if request.method == 'POST':
        # Crear un objeto ReconocimientoFacial con estado=False antes de iniciar el reconocimiento facial
        reconocimiento = ReconocimientoFacial.objects.create(estado=False)
        ...
        percent = num_recognitions * 10

        print(f"CEDULA MAS COMUN: {most_common_cedula}")
        print(f"RECONOCIDO: {nombre} {apellido}")
        print(f"PORCENTAJE DE RECONOCIMIENTOS: {percent} %")
        cap.release()
        cv2.destroyAllWindows()

        persona = Persona.objects.get(cedula=most_common_cedula)
        reconocimiento.estado = True
        reconocimiento.persona = persona
        reconocimiento.save()

        # Generar la URL para la vista 'bienvenido'
        url = reverse('reconfacial1:bienvenido', args=[nombre, apellido])
